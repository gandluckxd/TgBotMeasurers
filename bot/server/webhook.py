...
class WebhookProcessor:
    ...
    async def _create_measurement_from_lead(self, lead_id: int, lead_data: Dict[str, Any]):
        ...
        try:
            ...
            # будут получаться динамически из БД Altawin
            ...
            async for session in get_db():
                ...
                # Получаем зону доставки из Altawin для автораспределения
                delivery_zone_for_assignment = None
                if altawin_order_code:
                    from services.altawin import altawin_client
                    altawin_data = altawin_client.get_order_data(altawin_order_code)
                    if altawin_data:
                        delivery_zone_for_assignment = altawin_data.zone
                        logger.info(f"Получена зона доставки из Altawin: {delivery_zone_for_assignment}")

                # Создаем новый замер с автоматическим распределением
                # ВАЖНО: Теперь сохраняем только код заказа Altawin и минимум данных из AmoCRM
    ...
    async def _notify_admins_new_measurement(self, measurement):
        ...
        from bot_handlers.utils.notifications import send_new_measurement_to_admin, send_new_measurement_notification_to_observers
    ...
    async def _notify_measurer_new_assignment(self, measurement):
        ...
        from bot_handlers.utils.notifications import send_assignment_notification_to_measurer
    ...
    async def _notify_manager_new_assignment(self, measurement, manager):
        ...
        from bot_handlers.utils.notifications import send_assignment_notification_to_manager
